    ...
    return (
        <div className='flex items-center md:gap-4 gap-2 justify-between border border-black/10 p-2 w-full rounded-md'>
            ...
            <div className="flex flex-col items-start text-start md:w-[calc(100%-90px)] w-[calc(100%-72px)]">
                    <h3 className='line-clamp-1'>{cartItem.title}</h3>
                    <h3 className='md:hidden inline-flex'>{formatterUtility(Number(cartItem.price))}</h3>
            </div>
            <h3 className='md:inline-flex hidden'>{formatterUtility(Number(cartItem.price))}</h3>
            <button
                type='button'
